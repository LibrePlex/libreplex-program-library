...
use libreplex_metadata::{Collection, CreateMetadataInput, MetadataSummary};

use crate::{
    errors::ErrorCode, AttributeConfig, Creator, MintNumbers, MINT_NUMBERS_START,
};
...
pub fn handler(ctx: Context<Mint>) -> Result<()> {
    ...
    let metadata_summary = &ctx.accounts.metadata_summary;
    let receiver_token_account = &ctx.accounts.receiver_token_account;
    let mint = &ctx.accounts.mint;

    let receiver = &ctx.accounts.receiver;

    if &receiver_token_account.owner != receiver.key || &receiver_token_account.mint != mint.key {
    ...
    let create_ix_accounts = libreplex_metadata::cpi::accounts::CreateMetadataCtx {
        ...
        metadata_summary: metadata_summary.to_account_info(),
    };

    let mint_number = match creator.is_ordered {
        true => creator.minted,
        false => {
            let mint_numbers = &mut ctx
                .accounts
                .minter_numbers
                .as_ref()
                .ok_or(ErrorCode::MissingMintNumbers)?;
            ...
            select_mint_number(
                ...
                (creator.supply - creator.minted) as usize,
            )
        }
    };
    ...
    let create_ctx = CpiContext::new_with_signer(
        ctx.accounts.libreplex_metadata_program.to_account_info(),
        create_ix_accounts,
        signer_seeds.as_slice(),
    );

    let url_json = format!("{}{}.json", creator.asset_url, mint_number);
    ...
    let group_add_ctx = CpiContext::new_with_signer(
        ctx.accounts.libreplex_metadata_program.to_account_info(),
        group_add_accounts,
        signer_seeds.as_slice(),
    );
    ...
    let wrap_ctx = CpiContext::new(
        ctx.accounts.libreplex_nft_program.to_account_info(),
        libreplex_nft::cpi::accounts::WrapCtx {
            payer: ctx.accounts.payer.to_account_info(),
            authority: ctx.accounts.mint_authority.to_account_info(),
            mint: ctx.accounts.mint.to_account_info(),
...
pub fn select_mint_number(
    ...
    let chosen_index = (4
        * (u64::from_le_bytes(*array_ref![recent_slothash_data, 12, 8]) % (mints_left as u64))
        + (MINT_NUMBERS_START as u64)) as usize;
    ...
    mint_number_data[chosen_index..chosen_index + 4].copy_from_slice(&end_data);
