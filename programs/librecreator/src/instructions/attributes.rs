...
pub fn initialize(ctx: Context<InitializeAttributeConfig>, max_onchain_attribute_count: usize) -> Result<()> {
...
pub fn edit(ctx: Context<EditAttributeConfig>, input: EditAttributeConfigInput) -> Result<()> {
    ...
    input.update.iter().for_each(|(index, new_data)| {
        let edit_index = ATTRIBUTE_DATA_START + index * attribute_config.max_onchain_attribute_count;

        let new_data_slice = new_data.as_slice();

        attribute_config_data[edit_index..edit_index + attribute_config.max_onchain_attribute_count].copy_from_slice(new_data_slice);
    });


    input.add.iter().for_each(|new_data| {
        let edit_index = ATTRIBUTE_DATA_START + attribute_config.current * attribute_config.max_onchain_attribute_count;
