...
use libreplex::{Group, CreateMetadataInput, instructions::ExtendMetadataInput};
...
pub struct Mint<'info> {
    ...
    /// CHECK: checked in cpi
    #[account(mut)]
    pub metadata_permissions: AccountInfo<'info>,
...
pub fn handler(ctx: Context<Mint>) -> Result<()> {
    ...
    let create_ix_accounts = libreplex::cpi::accounts::CreateMetadata {
        metadata: ctx.accounts.metadata.to_account_info(),
        mint: ctx.accounts.mint.to_account_info(),
        permissions: ctx.accounts.metadata_permissions.to_account_info(),
    ...
    let asset = match &creator.asset_url {
        AssetUrl::JsonPrefix { url } => {
            libreplex::Asset::Json { url: format!("{}{}.json", url, mint_number) }
        },
        AssetUrl::ImagePrefix { url } => libreplex::Asset::Image { url: format!("{}{}.json", url, mint_number) },
        AssetUrl::ChainRenderer { program_id } => libreplex::Asset::ChainRenderer { program_id: *program_id },
    ...
    libreplex::cpi::create_metadata(create_ctx, CreateMetadataInput {
        name,
    ...
    let extend_ix_accounts = libreplex::cpi::accounts::ExtendMetadata {
        group: ctx.accounts.collection.to_account_info(),
        ...
        metadata_permissions: ctx.accounts.metadata_permissions.to_account_info(),
        mint: ctx.accounts.mint.to_account_info(),
        signer: creator.to_account_info(),
        system_program: ctx.accounts.system_program.to_account_info(),
    };
    ...
    let attributes = match creator.attribute_mappings {
        Some(_attribute_config) => {
            ...
            let attribute_index = ATTRIBUTE_DATA_START + 
                attribute_config_account.max_onchain_attribute_count * mint_number as usize;
            ...
            let attribute_slice = &attribute_account_data[attribute_index..attribute_index + attribute_config_account.max_onchain_attribute_count];
    ...
    libreplex::cpi::extend_metadata(extend_ctx, ExtendMetadataInput {
        attributes,
        royalties: None,
        invoked_permission: libreplex::PermissionType::Collection,
    })?;
