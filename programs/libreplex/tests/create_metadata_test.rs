...
mod metadata_tests {
    ...
    async fn create_metadata_ordinal_test() {
        let initial_data = vec![
            ...
            104, 0, 0, 16, 24, 96, 248, 80, 192, 128, 224, 32, 152,
            64, 128, 240, 184, 152, 56, 48, 144, 0, 136, 136, 104, 192, 128, 16, 24, 120, 248, 208,
            112, 128, 112, 104, 152, 0, 96, 240, 184, 24, 56, 48, 144, 0, 216, 8, 104, 128, 128,
        ...
        let append_to_ordinal_accounts =
            AppendToOrdinal {
                signer: authority,
                ordinal: ordinal.pubkey(),
                system_program: system_program::id()
        ...
        let append_to_ordinal_input = libreplex::instruction::AppendToOrdinal {
            input: AppendToOrdinalInput { append_data: append_data.clone() },
        };
        ...
        let final_account = context.banks_client.get_account(ordinal.pubkey()).await.unwrap();

        let mut expected_data:Vec<u8> = [&initial_data[..], &append_data[..]].concat();
